...
#include "../util/stencils_composable.h"
    ...
    if(axis == 0)
        cuda_compute
               (nsteps, d_data_in, d_data_out, offset,
                ...
                do_all_3d_2_gpu_acc<REAL_T, diffusion_3d>);
    else if(axis == 'x')
        cuda_compute
               (nsteps, d_data_in, d_data_out, offset,
                global_grid_size, blocks, threads_per_block, diffusion_3d(),
                do_all_3d_2_x_gpu_acc<REAL_T, diffusion_3d>);
    else if(axis == 'y')
        cuda_compute
               (nsteps, d_data_in, d_data_out, offset,
                global_grid_size, blocks, threads_per_block, diffusion_3d(),
                do_all_3d_2_y_gpu_acc<REAL_T, diffusion_3d>);
    else if(axis == 'z')
        cuda_compute
               (nsteps, d_data_in, d_data_out, offset,
                global_grid_size, blocks, threads_per_block, diffusion_3d(),
                do_all_3d_2_z_gpu_acc<REAL_T, diffusion_3d>);                                                                
    gpu_timer.stop();
